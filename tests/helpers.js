...
export const createTestDb = () => {
  ...
  db.exec(`
    ...
    CREATE VIEW ready_issues AS SELECT * FROM issues
      WHERE id NOT IN (SELECT d.issue_id FROM dependencies d
        JOIN issues i2 ON i2.id = d.depends_on_id
        WHERE d.type = 'blocks' AND i2.status <> 'closed');
    CREATE VIEW blocked_issues AS SELECT i.*, COUNT(d.depends_on_id) AS blocked_by_count
      ...
      JOIN issues i2 ON i2.id = d.depends_on_id AND i2.status <> 'closed'
      GROUP BY i.id;
  `)
...
export const seedIssues = (db) => {
  const insert = db.prepare(`INSERT INTO issues
    (id, title, description, design, acceptance_criteria, notes, status, priority, issue_type, assignee, created_at, updated_at, closed_at, metadata)
    VALUES (?, ?, ?, '', '', ?, ?, ?, ?, ?, ?, ?, ?, '{}')`)
